...
export default function NavLive() {
  ...
  const [noteBlocking, setNoteBlocking] = useState(false); // ✅ true => bouton Continuer visible
  ...
  useEffect(() => {
    ...
    if (hasNote && inNoteZone) {
      ...
      if (canShow && !suppressed) {
        ...
        if (blocking) {
          clearNoteTimer();
          setNoteBlocking(true);
          setPaused(true); // ✅ bloque la progression tant que l'overlay est affiché
          setActiveNote(noteRaw);
          if (audioOn) speakNoteTTS(noteRaw);
        } else {
          clearNoteTimer();
          setNoteBlocking(false);
          setPaused(true); // ✅ IMPORTANT: même en auto, on gèle l'arrivée/skip pour respecter la durée
          setActiveNote(noteRaw);
          if (audioOn) speakNoteTTS(noteRaw);
